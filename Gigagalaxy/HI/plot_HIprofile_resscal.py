...
def plot_bluediskprof(ax):
    ...
    for d in range(len(idcheck)):
        ...
        ax.plot(rad / R_HI, sigma, linestyle='solid', color='lightgray', lw=1.8)


def plot_HIprofile(runs, dirs, outpath, snap, suffix, nshells=140, zmax=5.0e-3):
    ...
    figure.set_axis_limits_and_aspect(xlim=[0.0, 2.0], ylim=[0., 15.0])
    figure.set_fontsize(10.)
    figure.set_axis_labels(xlabel=r"${\rm R/R_{HI}}$", ylabel=r"${\rm \Sigma_{HI} \,[M_{\odot}\,pc^{-2}]}$")
    ax = figure.axes[0]
    
    plot_bluediskprof(ax)
    
    # f = open("mass_diameter_res.txt", "w")
    f = open("mass_diameter_res.txt", "a")
    ...
    for d in range(len(runs)):
        ...
        s = gadget_readsnap(snapnum, snappath=dd + '/output/', hdf5=True, loadonlytype=[0],
                            loadonly=['pos', 'vel', 'mass', 'gmet', 'nh', 'u', 'rho', 'sfr', 'vol', 'gz'], forcesingleprec=True)
        sf = load_subfind(snapnum, dir=dd + '/output/', hdf5=True)
        ...
        ax.plot(rad / R_HI, sigma, linestyle=style[d], color='b', lw=1.8, label=label[d])
        ax.plot(rad / R_HI_GK, sigmaGK, linestyle=style[d], color='r', lw=1.5)
        
        # string = "$\\rm{R_{HI}}=%.1f$" % (1.0e3 * R_HI)
        # ax.text(0.45, 0.84, string, fontsize=6, transform=ax.transAxes )
        # string = "$\\rm{M_{HI}}=%.1fx10^9$" % (1.0e-9 * M_HI)
        # ax.text(0.45, 0.75, string, fontsize=6, transform=ax.transAxes )
    ...
    # limits = ax.get_ylim()  # ax.vlines( R_HI*1.0e3, limits[0], limits[1], linestyle='--', color='darkgray', lw=1.2 )
    
    titlestr = 'Au' + runs[0].split('_')[1]
    ax.text(0.94, 0.9, titlestr, fontsize=12, transform=ax.transAxes, horizontalalignment='right')
    ax.legend(loc='upper left', frameon=False, fontsize=figure.fontsize)
    
    savefig("%s/%s_HIprofile_%03dresscal.%s" % (outpath, runs[d], snap, suffix), dpi=300)
...
prefix5 = './level5_MHD/'
prefix4 = '/hits/universe/GigaGalaxy/level4_MHD/'
prefix3 = './level3_MHD/'
outpath = '../plots'
dirs = [prefix5, prefix4, prefix3]
runs = ['halo_16'] * len(dirs)
runs = ['halo_24'] * len(dirs)
runs = ['halo_6'] * len(dirs)
suffix = 'pdf'

plot_HIprofile(runs, dirs, outpath, snap, suffix, nshells=140)