...
def plot_stellarmass_vs_age(runs, dirs, outpath, nrows, ncols, outputlistfile, zlist, suffix, subhalo=0, restest=[5, 4], colorbymass=False,
                            dat=False):
    panels = nrows * ncols
    ...
    figure.set_axis_limits_and_aspect(xlim=[5.e8, 5.e11], ylim=[0., 14.], logxaxis=True)
    ...
    for isnap in range(nsnaps):
        ...
        if zlist[isnap] < 0.1:
            ...
            medianage = genfromtxt(table, comments='#', usecols=4)
            sigmadownage = genfromtxt(table, comments='#', usecols=5)
            sigmaupage = genfromtxt(table, comments='#', usecols=6)
            # put data in correct units
            masses = 10 ** masses
            medianage = 10 ** (medianage - 9.0)
            sigmadownage = 10 ** (sigmadownage - 9.0)
            sigmaupage = 10 ** (sigmaupage - 9.0)
            
            ax.fill_between(masses, sigmadownage, sigmaupage, color='royalblue', alpha=0.2, edgecolor='None')
            ax.semilogx(masses, medianage, color='royalblue', lw=1.)
            label = ["Gallazzi+ 05"]
        ...
        for d in range(len(runs)):
            ...
            mass_weighted_age = (sdata['age'] * sdata['mass']).sum() / mstars
            ...
            ax.semilogx(1.0e10 * mstars, mass_weighted_age, marker=marker, mfc='none', mec=color, ms=5., mew=1., linestyle='None', label=label)
    ...
    if restest:
        figure.fig.savefig("%s/stellarmass_vs_age%03d_restest.pdf" % (wpath, snaps[-1]), dpi=300)
    else:
        figure.fig.savefig("%s/stellarmass_vs_age%03d.pdf" % (wpath, snaps[-1]), dpi=300)