...
def plot_sfrvsradius(runs, dirs, outpath, snap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        sfr, radius = subhalos.get_sfr_vs_position()
        
        ax = axes([0.15 / fac, 0.13, 0.8 / fac, 0.8])
        
        ax.loglog(1.0e3 * radius, sfr, 'o', ms=3.0, color=colors[d], label="$\\rm{%s}$" % runs[d])
        ...
        xlabel("$\\rm{r\\,[kpc]}$")
        ylabel("$\\rm{SFR\\,[M_\\odot\\,yr^{-1}]}$")
        
        savefig("%s/sfrvsradius_%s.%s" % (outpath, runs[d], suffix), dpi=300)
        fig.clf()