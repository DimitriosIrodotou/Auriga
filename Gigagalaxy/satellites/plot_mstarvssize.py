...
def plot_mstarvssize(runs, dirs, outpath, snap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        stellarmass, size = subhalos.get_stellar_masses_vs_sizes()
        
        axes([0.17 / fac, 0.13, 0.8 / fac, 0.8])
        
        loglog(1.0e3 * size, 1.0e10 * stellarmass, 'o', ms=3.0, color=colors[d], label="$\\rm{%s}$" % runs[d])
        ...
        savefig("%s/mstarvssize_%s.%s" % (outpath, runs[d], suffix), dpi=300)
        fig.clf()