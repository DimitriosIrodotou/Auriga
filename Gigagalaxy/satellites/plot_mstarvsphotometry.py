...
def plot_mstarvsphotometry(runs, dirs, outpath, snap, suffix, band):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        starmass, photo = subhalos.get_mstar_vs_photometry(band=band_array[band])
        
        ax = axes([0.15 / fac, 0.13, 0.8 / fac, 0.8])
        ax.semilogy(photo, 1.0e10 * starmass, 'o', ms=3.0, color=colors[d], label="$\\rm{%s}$" % runs[d])
        ax.invert_xaxis()
        ...
        savefig("%s/mstarvsphotometry_%s_%s.%s" % (outpath, runs[d], band, suffix), dpi=300)
        fig.clf()
...
def plot_all_photometric_bands(runs, dirs, outpath, snap, suffix):
    for i in bands:
        plot_mstarvsphotometry(runs, dirs, outpath, snap, suffix, i)
