...
def plot_msfrgasvsradius(runs, dirs, outpath, snap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        print
        j, sfrmass, radius
        ax.loglog(1.0e3 * radius[j], 1.0e10 * sfrmass[j], 'o', ms=3.0, color=colors[d], label="$\\rm{%s}$" % runs[d])
        ...
        savefig("%s/msfrgasvsradius_%s.%s" % (outpath, runs[d], suffix), dpi=300)
        fig.clf()