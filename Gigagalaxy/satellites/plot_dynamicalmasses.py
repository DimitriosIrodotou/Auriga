...
def plot_dynamicalmasses(runs, dirs, outpath, snap, suffix, band='V'):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        ax.semilogy(magnitudecat[j], 1.0e10 * (massstar[j] + massgas[j] + massdm[j]), 's', ms=5.0, mec='None', mfc='black',
                    label="$\\rm{%s}$" % runs[d])
        
        plot_mdynamicalvsMv(ax, band)
        ...
        xlabel("$\\rm{M_{V}}$")
        ylabel("$\\rm{M_{dyn}(\\leq\,r_{h})\\,[M_{\\odot}]}$")
        
        savefig("%s/dynamicalmasses_%s_%s.%s" % (outpath, runs[d], band, suffix), dpi=300)
        fig.clf()


def plot_mdynamicalvsMv(ax, band):
    ...
    ax.semilogy(data.Vabsmagnitude[i], data.dynamicalmass[i], 'o', ms=5.0, mec='None', mfc='r', label='Milky Way')
    i, = np.where((data.associated == 'A') | (data.associated == 'A,L'))
    ax.semilogy(data.Vabsmagnitude[i], data.dynamicalmass[i], 'o', ms=5.0, mec='None', mfc='b', label='Andromeda')
    i, = np.where((data.associated == 'L') | (data.associated == 'L,G') | (data.associated == 'L,A'))
    ax.semilogy(data.Vabsmagnitude[i], data.dynamicalmass[i], 'o', ms=5.0, mec='None', mfc='g', label='Local Group')