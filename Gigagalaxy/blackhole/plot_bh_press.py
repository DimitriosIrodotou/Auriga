...
def plot_bh_press(runs, dirs, outpath, firstsnap, lastsnap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        for isnap in range(nsnaps):
            ...
            s = gadget_readsnap(snap, snappath=dd + '/output/', hdf5=True, loadonlytype=[5], loadonly=['pos', 'bhpr'])
            ...
            bhpress[isnap] = s.data['bhpr'][ibh[0]]
            time[isnap] = s.cosmology_get_lookback_time_from_a(s.time)
        
        # time = time[::-1]
        # bhhsml = bhhsml[::-1]
        
        ibh, = np.where(bhpress > 0.0)
        
        ax1.semilogy(time[ibh], bhpress[ibh], color=colors[d], label="$\\rm{%s-{%s}}$" % (runs[d].split('_')[0], runs[d].split('_')[1]))
    ...
    ax1.set_ylabel("$\\rm{P\\,[code units]}$")
    ...
    fig.savefig('%s/bh_press.%s' % (outpath, suffix), dpi=300)