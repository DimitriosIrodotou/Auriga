...
def plot_bh_rho(runs, dirs, outpath, firstsnap, lastsnap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        for isnap in range(nsnaps):
            ...
            s = gadget_readsnap(snap, snappath=dd + '/output/', hdf5=True, loadonlytype=[5], loadonly=['pos', 'bhro'])
            ...
            bhrho[isnap] = s.data['bhro'][ibh[0]]
            time[isnap] = s.cosmology_get_lookback_time_from_a(s.time)
        
        # time = time[::-1]
        # bhhsml = bhhsml[::-1]
        
        ibh, = np.where(bhrho > 0.0)
        
        ax1.semilogy(time[ibh], bhrho[ibh], color=colors[d], label="$\\rm{%s-{%s}}$" % (runs[d].split('_')[0], runs[d].split('_')[1]))
    ...
    ax1.set_ylabel("$\\rm{\\rho\\,[code units]}$")
    ...
    fig.savefig('%s/bh_rho.%s' % (outpath, suffix), dpi=300)