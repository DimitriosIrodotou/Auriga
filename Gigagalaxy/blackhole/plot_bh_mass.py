...
def plot_bh_mass(runs, dirs, outpath, firstsnap, lastsnap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        for isnap in range(nsnaps):
            ...
            s = gadget_readsnap(snap, snappath=dd + '/output/', hdf5=True, loadonlytype=[5], loadonly=['pos', 'bhma'])
            ...
            bhmass[isnap] = s.data['bhma'][ibh[0]]
        ...
        ax1.semilogy(time[ibh], bhmass[ibh], color=colors[d], linestyle='-', label="$\\rm{%s-{%s}}$" % (runs[d].split('_')[0], runs[d].split('_')[1]))
    ...
    fig.savefig('%s/bh_mass.%s' % (outpath, suffix), dpi=300)