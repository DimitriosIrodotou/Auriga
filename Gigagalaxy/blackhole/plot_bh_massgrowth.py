...
def plot_bh_massgrowth(runs, dirs, outpath, firstsnap, lastsnap, suffix):
    fac = 1.0
    ...
    time = zeros(nsnaps)
    bhmassquasar = zeros(nsnaps)
    bhmassradio = zeros(nsnaps)
    ...
    for d in range(len(runs)):
        ...
        for isnap in range(nsnaps):
            ...
            s = gadget_readsnap(snap, snappath=dd + '/output/', hdf5=True, loadonlytype=[5], loadonly=['pos', 'bcmq', 'bcmr'])
            ...
            bhmassquasar[isnap] = s.data['bcmq'][ibh[0]]
            bhmassradio[isnap] = s.data['bcmr'][ibh[0]]
            time[isnap] = s.cosmology_get_lookback_time_from_a(s.time)
        
        # time = time[::-1]
        # bhhsml = bhhsml[::-1]
        
        ibh, = np.where((bhmassquasar + bhmassradio) > 0.0)
        
        bhmassquasar[:] *= 1.0e10
        bhmassradio[:] *= 1.0e10
        
        ax1.semilogy(time[ibh], bhmassquasar[ibh], color=colors[d], linestyle='--')
        ax1.semilogy(time[ibh], bhmassradio[ibh], color=colors[d], linestyle=':')
        ax1.semilogy(time[ibh], bhmassquasar[ibh] + bhmassradio[ibh], color=colors[d],
                     label="$\\rm{%s-{%s}}$" % (runs[d].split('_')[0], runs[d].split('_')[1]), linestyle='-')
    ...
    fig.savefig('%s/bh_massgrowth.%s' % (outpath, suffix), dpi=300)