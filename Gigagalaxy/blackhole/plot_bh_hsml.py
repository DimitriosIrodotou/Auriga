...
def plot_bh_hsml(runs, dirs, outpath, firstsnap, lastsnap, suffix):
    fac = 1.0
    ...
    for d in range(len(runs)):
        ...
        for isnap in range(nsnaps):
            ...
            s = gadget_readsnap(snap, snappath=dd + '/output/', hdf5=True, loadonlytype=[5], loadonly=['pos', 'bhhs'])
            ...
            bhhsml[isnap] = s.data['bhhs'][ibh[0]]
            time[isnap] = s.cosmology_get_lookback_time_from_a(s.time)
        
        # time = time[::-1]
        # bhhsml = bhhsml[::-1]
        
        ibh, = np.where(bhhsml > 0.0)
        
        ax1.plot(time[ibh], 1.0e3 * bhhsml[ibh], color=colors[d], label="$\\rm{%s-{%s}}$" % (runs[d].split('_')[0], runs[d].split('_')[1]))
    ...
    ax1.set_xlim(0.0, 14.0)
    ax1.invert_xaxis()
    ax1.set_xlabel("$\\rm{t_{look}\\,[Gyr]}$")
    ax1.set_ylabel("$\\rm{hsml\\,[kpc]}$")
    ...
    fig.savefig('%s/bh_hsml.%s' % (outpath, suffix), dpi=300)