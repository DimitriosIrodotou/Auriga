...
def bar_strength(pdf, data, levels, z):
    ...
    for il in range(nlevels):
        ...
        for s in data:
            ...
            istars, = np.where((s.type == 4) & (age > 0.) & (s.r() * 1e3 < 30.0) & (abs(s.pos[:, 2] * 1e3) < 3.0))  # Select stars.
            x_prc, y_prc, z_prc = s.pos[istars, 2] * 1000, s.pos[istars, 1] * 1000, s.pos[
                istars, 0] * 1000  # Load positions and convert from Mpc to Kpc.

            nbins = 50  # Number of radial bins.
            r = np.sqrt(x_prc[:] ** 2 + y_prc[:] ** 2)  # Radius of each particle.
            ...
            for i in range(0, nbins):
                ...
                xfit = x_prc[(r < r_b) & (r > r_s) & (abs(z_prc) < 0.8)]
                yfit = y_prc[(r < r_b) & (r > r_s) & (abs(z_prc) < 0.8)]
