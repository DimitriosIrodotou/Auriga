    ...
    for radial_cut_min, radial_cut_max, top_axis, middle_axis, bottom_axis in zip(radial_cuts_min, radial_cuts_max,
        ...
        for i in range(len(names)):
            ...
            if i == 0:
                ...
                net_flow = plot_tools.resample_linear1D(net_flow, len(original_net_flow))
                lookback_times_gfml = plot_tools.resample_linear1D(lookback_times_gfml, len(original_net_flow))
                bottom_axis.plot(lookback_times_gfml,
                    np.divide(net_flow - original_net_flow, original_net_flow), color=colors[i])
