...
def gas_movie(data, read):
    ...
    if read is True:
        ...
        for redshift in np.flip(redshifts[np.where(redshifts <= redshift_cut)]):
            ...
            for s in data:
                ...
                temperature_face_on = \
                    s.get_Aslice("temprho", res=res, axes=[1, 2], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                rho_face_on = \
                    s.get_Aslice("rho", res=res, axes=[1, 2], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                temperature_edge_on = \
                    s.get_Aslice("temprho", res=res, axes=[1, 0], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                rho_edge_on = \
                    s.get_Aslice("rho", res=res, axes=[1, 0], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]

                # Get the radial velocity projections #
                cylindrical_radius = np.sqrt(np.sum(s.data['pos'][gas_mask, 1:] ** 2, axis=1))  # In Mpc.
                s.data['vrad'] = np.divide(np.sum(s.data['vel'][gas_mask, 1:] * s.data['pos'][gas_mask, 1:], axis=1),
                    cylindrical_radius)
                vrad_face_on = \
                    s.get_Aslice("vrad", res=res, axes=[1, 2], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                ...
                vrad_edge_on = \
                    s.get_Aslice("vz", res=res, axes=[1, 0], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                ...
                pressure_face_on = \
                    s.get_Aslice("Ptherm", res=res, axes=[1, 2], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
                pressure_edge_on = \
                    s.get_Aslice("Ptherm", res=res, axes=[1, 0], box=[boxsize, boxsize], proj=True, proj_fact=0.05,
                        numthreads=8)["grid"]
    ...
    for i in range(len(names)):
        ...
        temperature_face_on = np.load(
            path + 'temperature_face_on_3000_' + str(re.split('_|.npy', names[i])[2]) + '.npy')
        temperature_edge_on = np.load(
            path + 'temperature_edge_on_3000_' + str(re.split('_|.npy', names[i])[2]) + '.npy')
        ...
        temperature = axis00.pcolormesh(x, y, (temperature_face_on / rho_face_on).T,
            norm=matplotlib.colors.LogNorm(vmin=3e3, vmax=5e7), cmap='ocean', rasterized=True)
        vrad = axis01.pcolormesh(x, y, vrad_face_on.T, cmap='bwr', vmin=-5e3, vmax=5e3, rasterized=True)
        rho = axis10.pcolormesh(x, y, (rho_face_on * boxsize * 1e3).T,
            norm=matplotlib.colors.LogNorm(vmin=1e6, vmax=1e10), cmap='magma', rasterized=True)
        pressure = axis11.pcolormesh(x, y, pressure_face_on.T, norm=matplotlib.colors.LogNorm(vmin=1e3, vmax=1e8),
            cmap='cubehelix', rasterized=True)
        ...
        # Plot the projections #
        temperature = axis00.pcolormesh(x, y, (temperature_edge_on / rho_edge_on).T,
            norm=matplotlib.colors.LogNorm(vmin=3e3, vmax=5e7), cmap='ocean', rasterized=False)
        vrad = axis01.pcolormesh(x, y, vrad_edge_on.T, cmap='bwr', vmin=-3.5e4, vmax=3.5e4, rasterized=True)
        rho = axis10.pcolormesh(x, y, (rho_edge_on * boxsize * 1e3).T,
            norm=matplotlib.colors.LogNorm(vmin=1e6, vmax=1e10), cmap='magma', rasterized=True)
        pressure = axis11.pcolormesh(x, y, pressure_edge_on.T, norm=matplotlib.colors.LogNorm(vmin=1e3, vmax=1e8),
            cmap='cubehelix', rasterized=True)
